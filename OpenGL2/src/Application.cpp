...
public:
	...
	MainGame() {
		...
		//glBufferData(GL_ARRAY_BUFFER, 6 * 6 * 9 * sizeof(float), vertices, GL_STATIC_DRAW);
		...
		InstancedVB ivb(&translations[0],6*sizeof(float),GL_STATIC_DRAW);
		//unsigned int instanceVBO;
		//glGenBuffers(1, &instanceVBO);
		//glBindBuffer(GL_ARRAY_BUFFER, instanceVBO);
		//glBufferData(GL_ARRAY_BUFFER, sizeof(float) * 6, &translations[0], GL_STATIC_DRAW);
		...
		//glBindBuffer(GL_ARRAY_BUFFER, ivb.m_RendererID); // this attribute comes from a different vertex buffer
		...
		while (!glfwWindowShouldClose(window.window))
		{
			...
			/*glBindVertexArray(quadVAO);*/
			va.Bind();
			glDrawArraysInstanced(GL_TRIANGLES, 0, 36, 2); // 100 triangles of 6 vertices each
			//glBindVertexArray(0);
